...
contract ArtworkMarket is ERC721Full, Ownable {
    ...
    function registerArt(string memory uri, uint expiryTime) public payable onlyOwner {
        token_ids.increment();
        uint token_id = token_ids.current();
        _mint(foundation_address, token_id);
        _setTokenURI(token_id, uri);
        createAuction(token_id, expiryTime);
    }
    function createAuction(uint token_id, uint expiryTime) public onlyOwner {
        auctions[token_id] = new ArtworkAuction(foundation_address, expiryTime);
    }
